...
- [x] 收藏服务和收藏夹管理。
- [x] 历史服务和继续阅读。
- [x] 下载基础链路：队列、取消、重试、移除、离线快照和本地阅读。
- [x] 图片缓存和阅读快照缓存。
- [ ] 下载章节选择页。
- [ ] 下载任务详情、进度统计、恢复和删除文件管理。
...
- Rust：认证、签名、请求头、分页、分类/排行/搜索/详情/章节/图片解析。
- Flutter：`BikaProvider`、Provider 注册、设置入口、登录态展示和模型映射。
- 下载/阅读：通过现有 `getChapters`、`getChapterImages`、`getReaderChapterSnapshot` 契约复用。
